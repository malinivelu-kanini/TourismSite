    ...
    public class TravelAgentRepo : IRepo<TravelAgent,int>
    {
        ...
        public async Task<TravelAgent?> Delete(int key)
        {
            try
            {
                ...
                if (travelagent != null)
                {
                    _context.TravelAgent.Remove(travelagent);
                    await _context.SaveChangesAsync();
                    return travelagent;
                }
            }
        ...
        public async Task<TravelAgent?> Get(int key)
        {
            try
            {
                var travelagent = await _context.TravelAgent.Include(i => i.Users).FirstOrDefaultAsync(i => i.TravelAgentId == key);
                return travelagent;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ICollection<TravelAgent>>GetAll()
        {
            try
            {
                var travelagent = await _context.TravelAgent.Include(d => d.Users).ToListAsync();
        ...
        public async Task<TravelAgent?> Update(TravelAgent item)
        {
            try
            {
                var travelagent = _context.TravelAgent.FirstOrDefault(u => u.TravelAgentId == item.TravelAgentId); ;
                if (travelagent != null)
                {
