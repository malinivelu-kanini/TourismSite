...
namespace LoginandSignup.Services
{
    public class AdminRepo : IRepo<Admin, int>
    {
        ...
        public async Task<Admin?> Get(int key)
        {
            try
            {
                var user = await _context.Admin.FirstOrDefaultAsync(u => u.AdminId == key);
                return user;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ICollection<Admin>?> GetAll()
        {
            try
            {
                var users = await _context.Admin.ToListAsync();
                if (users.Count > 0)
                    return users;
            }
            catch (Exception ex)
        ...
        public async Task<Admin?> Update(Admin item)
        {
            try
            {
                var existingAdmin = await _context.Admin.FirstOrDefaultAsync(u => u.AdminId == item.AdminId);
        ...
        public async Task<Admin?> Delete(int key)
        {
            try
            {
                var existingAdmin = await _context.Admin.FirstOrDefaultAsync(u => u.AdminId == key);
                if (existingAdmin != null)
                {
